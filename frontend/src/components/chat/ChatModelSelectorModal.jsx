...
import { 
  ...
  DollarSign, MessageSquare, Sparkles
} from 'lucide-react';
...
export default function ChatModelSelectorModal({ models, selectedModel, onSelect, onClose }) {
  const [activeProvider, setActiveProvider] = useState('all');
  const [searchQuery, setSearchQuery] = useState('');
  const [sortBy, setSortBy] = useState('name'); // name, cost-low, cost-high
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium whitespace-nowrap transition-colors ${
                activeProvider === provider
                  ...
                  : 'text-[var(--text-muted)] hover:text-white hover:bg-[#1a1c25]'
              }`}
            >
        ...
        <div className="flex items-center gap-4 px-4 py-3 border-b border-[var(--border-color)]">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full bg-[var(--bg-primary)] border border-[#2a2c35] rounded-lg pl-10 pr-4 py-2 text-sm outline-none focus:border-emerald-500"
            />
          </div>
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="bg-[var(--bg-primary)] border border-[#2a2c35] rounded-lg px-3 py-2 text-sm outline-none"
            >
                ...
                <button
                  ...
                  className={`relative p-4 rounded-xl text-left transition-all border ${
                    isSelected
                      ...
                      : 'bg-[var(--bg-primary)] border-[var(--border-color)] hover:border-[#2a2c35] hover:bg-[var(--bg-secondary)]'
                  }`}
                >
                  {/* Selected check */}
                  {isSelected && (
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-3">
                    ...
                    {caps.maxContext && (
                      <span className="text-xs bg-[#1a1c25] text-[var(--text-muted)] px-1.5 py-0.5 rounded">
