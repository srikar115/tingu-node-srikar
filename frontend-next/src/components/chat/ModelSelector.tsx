...
import { 
  ChevronDown, Check, Eye, Globe, Brain
} from 'lucide-react';
...
function getPriceTier(model: Model): { label: string; color: string } {
  const avgCost = ((model.inputCost || 0) + (model.outputCost || 0)) / 2;
...
export function ModelSelector({ models, selectedModel, onSelect, disabled }: ModelSelectorProps) {
  ...
  return (
    <div className="relative">
      ...
      >
        {selectedModel && (
          <div className={`w-2 h-2 rounded-full bg-gradient-to-r ${PROVIDER_COLORS[provider] || 'from-gray-500 to-slate-600'}`} />
        )}
            ...
            <div className="max-h-96 overflow-y-auto">
              {models.map(model => {
                ...
                return (
                  ...
                  >
                    <div className={`w-2 h-2 rounded-full mt-2 bg-gradient-to-r ${PROVIDER_COLORS[modelProvider] || 'from-gray-500 to-slate-600'}`} />
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-xs text-[var(--text-muted)] mt-1">
                        In: ${model.inputCost || 0}/1K · Out: ${model.outputCost || 0}/1K
                      </div>
                    </div>
